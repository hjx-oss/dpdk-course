...
def cases():
    ...
    with socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(3)) as raw:
        ...
        invalid = {
            ...
            "other UDP port": frame(port=9001),
...
if __name__ == "__main__":
    if "--inside" not in sys.argv:
        demo.lab.enter([sys.executable, str(Path(__file__).resolve()), "--inside"], static_neighbor=True)
    lines, result = demo.run(cases)
    idle_lines, idle_result = demo.run(lambda: ["PASS idle exit and port reopen"])
    assert "received=0" in idle_result and "sent=0" in idle_result
    record = {"status": "passed", "dpdk": "25.11.3", "checks": lines + idle_lines,
              "result": result, "idle_result": idle_result}
    (demo.LESSON / "build/check.json").write_text(json.dumps(record, ensure_ascii=False, indent=2) + "\n")
