# 第六集：ICMP Echo：让用户态协议栈响应 ping

承接第五集的 ARP 主动查询、应答、邻居表和 UDP，把共同的 IPv4 检查独立出来，再按 Protocol 分到 UDP 或 ICMP。新增 ICMP Echo 应答，系统自带的 ping 可以收到回复。
...
| [src/arp.h](src/arp.h)、[src/arp.c](src/arp.c) | 继承第五集完整 ARP 模块：主动查询、应答、缓存、过期与重试 |
| [src/ipv4.h](src/ipv4.h)、[src/ipv4.c](src/ipv4.c) | 检查共同的 IPv4 头部与长度，按 Protocol 分发 |
| [src/icmp.h](src/icmp.h)、[src/icmp.c](src/icmp.c) | Echo Request 改成 Echo Reply，校验并重算校验和 |
...
不需要额外参数。脚本加载环境、编译程序、创建隔离 veth、启动服务端，验证后停止自己的进程并退出。脚本先调用 Linux ping 发送三次探测，再发一条 UDP 消息；随后回归主动 ARP 查询及三次重试失败。输出包括：

```text
PASS ping: 3 Echo requests, 3 Echo replies
PASS hello-after-ping: reply matches
PASS ARP neighbor learned without static entry
RESULT received=5 sent=5 ignored=0 learned=0 unsent=0 mbufs_in_use=0
...
export RUNTIME_DIRECTORY="$(mktemp -d "$PWD/06-icmp/build/runtime.XXXXXX")"
./06-icmp/build/icmp_udp \
  -l "$DPDK_CPU" \
  -m 128 \
  --no-pci \
  --no-huge \
  ...
  > 06-icmp/build/manual-server.log 2>&1 &
server_pid=$!
...
```bash
cat 06-icmp/build/manual-server.log
```
...
```bash
ping -n -c 3 -i 0.2 -W 2 192.0.2.2
```

`-n` 直接显示数字地址；`-c 3` 发送三次；`-i 0.2` 间隔 0.2 秒；`-W 2` 等待回复最多两秒。预期三次都有回复、丢包率为 0%。`icmp_seq` 是探测序号，`time` 是往返时间；不是程序处理一张包的 CPU 时间。
...
source ./env.sh
make -C 06-icmp
python3 06-icmp/tests/resolve.py
```
...
```bash
source ./env.sh
make -C 06-icmp check
```

检查主动广播请求与真实 Linux 应答、无人应答时的三次重试，以及 ARP 应答字段与填充、错误地址和格式、UDP 长度与校验和、空正文、二进制、最大普通 MTU 载荷、错误输入后的连续收发、空闲退出和端口重新打开。还检查 ICMP 奇数字节正文、标识符和序号保持、IP/ICMP 校验和、尾部填充，以及错误消息类型、长度和分片。
...
IPv4 只支持无 VLAN、20 字节 IP 头，不支持 IP 选项和分片。UDP 只回显本机指定端口；IPv4 UDP 校验和为零的请求可接收，回复重新计算校验和。ICMP 只回复 type 8、code 0 的 Echo Request，回复为 type 0；identifier、sequence 和正文保留。不实现 ICMP 差错消息。不做路由或 TCP。
...
- 邻居项已学习但应用超时：检查后续 IPv4、ICMP 或 UDP 分支和目标端口。

协议定义见 [RFC 826：ARP](https://www.rfc-editor.org/rfc/rfc826)、[RFC 792：ICMP](https://www.rfc-editor.org/rfc/rfc792)和 [RFC 1071：Internet checksum](https://www.rfc-editor.org/rfc/rfc1071)。
