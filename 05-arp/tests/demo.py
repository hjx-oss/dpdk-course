...
def arguments():
    ...
    return [str(LESSON / "build/arp_udp"), "-l", str(cpu), "-m", "128",
...
def demonstrate():
    assert not arp_cases.neighbor(), "Expected an empty neighbor table"
    lines = ["PASS start: no static neighbor entry"]
    lines += exchange([b"hello-1", b"hello-2", b"hello-3"])
    entries = arp_cases.neighbor()
    assert len(entries) == 1 and entries[0]["lladdr"] == "02:00:00:00:00:02"
    assert "PERMANENT" not in entries[0].get("state", [])
    lines.append("PASS learned: 192.0.2.2 -> 02:00:00:00:00:02")
    return lines
